...
exports.streamSingleSection = streamSingleSection;
exports.streamCoherenceCheck = streamCoherenceCheck;
...
function buildContext(analysis, targetStack) {
    ...
    const envVarLines = analysis.envVarInventory?.length
        ? analysis.envVarInventory.slice(0, 30).map((v) => `  ${v.name}${v.defaultValue !== undefined ? ` (default: "${v.defaultValue}")` : ''} — ${v.files.slice(0, 3).join(', ')}${v.files.length > 3 ? ` +${v.files.length - 3} more` : ''}`).join('\n')
        : null;
    ...
    const depUsageLines = (() => {
        ...
        const parts = [];
        if (heavy.length) {
            parts.push(`  Heavily imported (≥5 files): ${heavy.slice(0, 8).join(', ')}`);
        }
        if (unused.length) {
            parts.push(`  Possibly unused (no imports detected): ${unused.slice(0, 8).join(', ')}`);
        }
 ...
 */
async function streamMigrationPlan(analysis, targetStack, options, onChunk, cancellationToken, onSectionStart) {
    const model = await selectModel();
    ...
    for (let i = 0; i < sections.length; i++) {
        ...
        let sectionContent = '';
        const captureChunk = (c) => { sectionContent += c; onChunk(c); };
        let succeeded = false;
 ...
 */
async function streamSingleSection(analysis, targetStack, options, sectionHeading, onChunk, cancellationToken) {
    const model = await selectModel();
    const context = buildContext(analysis, targetStack);
    const metadataContext = buildContext({ ...analysis, keyFiles: [] }, targetStack);
    const sections = buildSections(analysis, targetStack, options);
    const norm = (s) => s.replace(/^#+\s*/, '').toLowerCase().trim();
    const target = sectionHeading ? norm(sectionHeading) : '';
    const section = sections.find((s) => norm(s.heading) === target || norm(s.heading).includes(target) || target.includes(norm(s.heading)));
    if (!section) {
        return false;
    }
    const sysPrefix = `You are a senior software architect producing one section of a professional migration plan. ` +
    ...
    const prompt = `${sysPrefix}\n\n${context}\n\n---\n\n${section.ask}`;
    const metadataPrompt = `${sysPrefix}\n\n${metadataContext}\n\n---\n\n${section.ask}`;
    const minimalPrompt = `Write the "${norm(section.heading)}" section of a migration plan from ` +
        `${analysis.detectedStack.framework} to ${targetStack}. Be specific and include code examples.`;
    let content = '';
    const capture = (c) => { content += c; onChunk(c); };
 ...
 */
async function streamCoherenceCheck(analysis, targetStack, fullPlan, onChunk, cancellationToken) {
    const model = await selectModel();
    const { safe: safeTree } = filterFileTree(analysis.fileTree);
    ...
    const envVarCheckContext = analysis.envVarInventory?.length
        ...
        : '';
    const prompt = `You are a senior software architect performing a quality-assurance review of a migration plan that was just auto-generated by an AI. Your job is to catch problems before the developer acts on the plan.
...
One sentence: how many total issues were found across all categories and whether the plan is safe to act on as-is, or needs corrections first.`;
    const fallbackPrompt = `Review this migration plan and list: (1) any file paths that don't exist in the real project, (2) contradictions between sections, (3) version mismatches against the real deps.
...
${fullPlan.slice(0, 6000)}

Output as: ## Plan Coherence Review with sub-headings for each issue type.`;
    const minimalPrompt = `Briefly review this migration plan for contradictions and invented file names. Current stack: ${detectedStack.framework}. Target: ${targetStack}. Output as: ## Plan Coherence Review`;
