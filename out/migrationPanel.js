...
class MigrationPanel {
    ...
    async _runAnalysis(repoUrl, githubToken) {
        ...
        const config = vscode.workspace.getConfiguration('migrationAssistant');
        const token = githubToken || config.get('githubToken') || undefined;
        try {
    ...
    async _runMigrationPlan(analysis, targetStack, options) {
        ...
        try {
            await (0, copilotService_1.streamMigrationPlan)(analysis, targetStack, options, (chunk) => this._post({ type: 'planChunk', chunk }), token);
