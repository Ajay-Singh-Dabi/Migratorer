...
class MigrationPanel {
    ...
    async _runMigrationPlan(analysis, targetStack, options) {
        ...
        try {
            const failedSections = await (0, copilotService_1.streamMigrationPlan)(analysis, targetStack, options, (chunk) => {
                this._lastPlan += chunk;
                this._post({ type: 'planChunk', chunk });
            }, token, (index, total, heading) => {
                this._post({
    ...
    async applyPendingPlanPatch(regenerate = false) {
        ...
        if (regenerate) {
            ...
            this._post({ type: 'planChunk', chunk: '' }); // clear signal
            try {
                const regenFailed = await (0, copilotService_1.streamMigrationPlan)(this._lastAnalysis, // already has chunkSummaries from the first run
    ...
    async _retrySection(sectionHeading) {
        ...
        let retryContent = '';
        try {
            const ok = await (0, copilotService_1.streamSingleSection)(analysis, targetStack, options, sectionHeading, (chunk) => {
                retryContent += chunk;
                this._post({ type: 'planPatchChunk', chunk });
            }, token);
            if (ok) {
