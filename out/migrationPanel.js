...
class MigrationPanel {
    ...
    async _validateToken(githubToken) {
    ...
    _githubUserCheck(token) {
        return new Promise((resolve, reject) => {
            const https = require('https');
            const req = https.request({ hostname: 'api.github.com', path: '/user', headers: { 'User-Agent': 'vscode-migration-assistant/1.0', Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json' } }, (res) => {
                let data = '';
                res.on('data', (c) => (data += c));
                res.on('end', () => {
                    if (res.statusCode === 200) {
                        resolve(JSON.parse(data).login);
                    }
                    else {
                        reject(new Error(`Token invalid (HTTP ${res.statusCode})`));
                    }
                });
            });
            req.on('error', reject);
    ...
    async _exportPlan(format) {
        ...
        try {
            let content = '';
            await (0, copilotService_1.streamExportFormat)(this._lastPlan, format, this._lastAnalysis.repoInfo.repo, 'target stack', (chunk) => { content += chunk; this._post({ type: 'exportReady', exportFormat: format, exportContent: content }); }, this._cancellationSource.token);
        }
        catch (err) {
            this._post({ type: 'error', message: err.message });
        }
    ...
    async _runProgressCheck(compareBranch) {
        ...
        const { owner, repo, hostname } = (0, githubAnalyzer_1.parseGitHubUrl)(`https://${this._lastAnalysis.repoInfo.owner}/${this._lastAnalysis.repoInfo.repo}`);
        const storedToken = await this._secrets.get(TOKEN_SECRET_KEY);
    ...
    async _generateReport(targetStack, format) {
        ...
        let fullReport = '';
        try {
            await (0, copilotService_1.streamDetailedReport)(this._lastAnalysis, resolvedTarget, (chunk) => { fullReport += chunk; }, token);
        }
        catch (err) {
