...
function filterFileTree(fileTree: string[]): { safe: string[]; removed: number } {
...
async function selectModel(): Promise<vscode.LanguageModelChat> {
  ...
  throw new Error(
    'No GitHub Copilot models are available.\n' +
    'Please ensure:\n' +
...
function maskPrivatePackages(deps: Record<string, string>): Record<string, string> {
  const config = vscode.workspace.getConfiguration('migrationAssistant');
  if (!config.get<boolean>('maskOrgPackages', true)) { return deps; }
...
function buildContext(analysis: RepoAnalysis, targetStack: string): string {
  ...
  try {
    ...
    for await (const fragment of response.text) {
      ...
      if (buffer.length >= 300) {
        ...
        if (isRefusal(buffer)) { return false; }
...
async function streamWithFallback(
  ...
  if (await tryStream(model, fallbackPrompt, onChunk, cancellationToken)) { return; }
...
export async function streamStackRecommendations(
  analysis: RepoAnalysis,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
): Promise<void> {
...
export async function detectStackWithAI(
  analysis: RepoAnalysis,
  cancellationToken: vscode.CancellationToken
): Promise<import('./types').AIDetectedStack> {
...
export async function streamStackHealthAnalysis(
  analysis: RepoAnalysis,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
): Promise<void> {
...
export async function streamDebugHelp(
  analysis: RepoAnalysis,
  errorMessage: string,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
...
export async function streamExecSummary(
  analysis: RepoAnalysis,
  targetStack: string,
  plan: string,
  onChunk: (chunk: string) => void,
...
export async function streamFilePreviews(
  analysis: RepoAnalysis,
  targetStack: string,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
  ...
  const fileDump = keyFiles.slice(0, 6).map(f =>
    `### CURRENT: ${f.path}\n\`\`\`\n${f.content.slice(0, 1500)}\n\`\`\``
  ).join('\n\n');
...
export async function streamProgressCheck(
  analysis: RepoAnalysis,
  targetStack: string,
  plan: string,
  branchDiff: import('./types').BranchDiff,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
): Promise<void> {
...
export async function streamExportFormat(
  plan: string,
  ...
  const formatInstructions: Record<import('./types').ExportFormat, string> = {
...
export async function streamDetailedReport(
  analysis: RepoAnalysis,
  targetStack: string,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
  ...
  const sections: Array<{ heading: string; ask: string }> = [
    {
...
export async function streamJiraStories(
  analysis: RepoAnalysis,
  targetStack: string,
  plan: string,
  config: import('./types').JiraStoriesConfig,
  onChunk: (chunk: string) => void,
  cancellationToken: vscode.CancellationToken
): Promise<import('./types').JiraStory[]> {
  const model = await selectModel();
  ...
  const allStories: import('./types').JiraStory[] = [];

  const epicSections: Array<{ epicName: string; ask: string }> = [
    {
...
Output ONLY a valid JSON array — no commentary before or after.`;
    ...
    try {
      ...
      const jsonMatch = raw.match(/\[[\s\S]*\]/);
      if (jsonMatch) {
        const stories: any[] = JSON.parse(jsonMatch[0]);
        for (const s of stories) {
          storyCounter++;
          const story: import('./types').JiraStory = {
            epicKey,
  ...
  if (allStories.length > 0) {
    ...
    const pointsBySprint: Record<number, number> = {};
...
export async function generatePresets(
  analysis: RepoAnalysis,
  cancellationToken: vscode.CancellationToken
): Promise<import('./types').MigrationPreset[]> {
  const { detectedStack, repoInfo, totalFiles } = analysis;
...
Order presets from LEAST to MOST effort (incremental upgrade first, full rewrite last). Be specific.`;
  ...
  return JSON.parse(jsonMatch[0]) as import('./types').MigrationPreset[];
}
 ...
 */
export async function detectStackChangeIntent(
  userMessage: string,
  currentPlan: string,
  cancellationToken: vscode.CancellationToken
 ...
 */
export async function streamPlanPatch(
  analysis: RepoAnalysis,
  currentPlan: string,
  intent: StackChangeIntent,
  onChunk: (chunk: string) => void,
  onDone: (patchedPlan: string) => void,
  cancellationToken: vscode.CancellationToken
): Promise<void> {
  ...
  const affectedNorm = new Set(
    intent.affectedSections.map((h) => h.toLowerCase().trim())
  );
  ...
  for (const section of rawSections) {
    ...
    const isAffected =
      affectedNorm.has(firstLine.toLowerCase()) ||
    ...
    const prompt =
`You are rewriting one section of a migration plan because the user has decided to use "${intent.toComponent}" instead of "${intent.fromComponent}".
