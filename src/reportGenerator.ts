import {
  ...
  AlignmentType, PageBreak, ShadingType, Header, Footer,
...
function tokenize(markdown: string): Token[] {
  ...
  while (i < lines.length) {
    ...
    // Headings
    const h1 = line.match(/^# (.+)/);
    if (h1) { tokens.push({ kind: 'h1', text: h1[1].trim() }); i++; continue; }
    const h2 = line.match(/^## (.+)/);
    if (h2) { tokens.push({ kind: 'h2', text: h2[1].trim() }); i++; continue; }
    const h3 = line.match(/^### (.+)/);
    if (h3) { tokens.push({ kind: 'h3', text: h3[1].trim() }); i++; continue; }

    // Horizontal rule
    if (/^---+$/.test(line.trim())) { tokens.push({ kind: 'hr' }); i++; continue; }
    ...
    // Paragraph
    tokens.push({ kind: 'para', text: line }); i++;
  }
...
function parseInline(text: string): TextRun[] {
  ...
  for (const part of parts) {
    if (part.startsWith('`') && part.endsWith('`')) {
      ...
      runs.push(new TextRun({ text: part.slice(2, -2), bold: true, color: TEXT_DARK }));
    } else if (part.startsWith('*') && part.endsWith('*')) {
      runs.push(new TextRun({ text: part.slice(1, -1), italics: true }));
    } else if (part) {
      runs.push(new TextRun({ text: part, color: TEXT_DARK }));
    }
  }
...
function heading1(text: string): Paragraph {
  return new Paragraph({
    ...
    children: [new TextRun({ text: text.replace(/[#🔍🗺📋🔧💡🧪⚙🐳⚠✅📄📊🎯🏢📈]/gu, '').trim(), bold: true, color: BRAND_BLUE, size: 32 })],
...
function heading2(text: string): Paragraph {
  return new Paragraph({
    ...
    children: [new TextRun({ text: text.replace(/[🔍🗺📋🔧💡🧪⚙🐳⚠✅📄📊🎯🏢📈]/gu, '').trim(), bold: true, color: ACCENT_TEAL, size: 26 })],
...
function heading3(text: string): Paragraph {
  return new Paragraph({
    ...
    children: [new TextRun({ text: text.replace(/[🔍🗺📋🔧💡🧪⚙🐳⚠✅📄📊🎯🏢📈]/gu, '').trim(), bold: true, color: TEXT_DARK, size: 22 })],
...
function bulletPara(text: string, level = 0): Paragraph {
  return new Paragraph({
    bullet: { level },
    children: parseInline(text),
...
function numberedPara(text: string, idx: number): Paragraph {
  return new Paragraph({
...
function codeBlock(lines: string[], lang: string): (Paragraph | Table)[] {
  const langLabel = lang ? new Paragraph({
    children: [new TextRun({ text: lang.toUpperCase(), size: 14, color: TEXT_MUTED, bold: true })],
  ...
  const codePara = new Paragraph({
    ...
    shading: { type: ShadingType.SOLID, color: CODE_BG, fill: CODE_BG },
    border: {
      top:    { color: 'CBD5E1', size: 4, style: BorderStyle.SINGLE },
      bottom: { color: 'CBD5E1', size: 4, style: BorderStyle.SINGLE },
      left:   { color: ACCENT_TEAL, size: 12, style: BorderStyle.SINGLE },
      right:  { color: 'CBD5E1', size: 4, style: BorderStyle.SINGLE },
    },
  ...
  const headerRow = new TableRow({
    ...
    children: headers.map((h) =>
      new TableCell({
        ...
        shading: { type: ShadingType.SOLID, color: TABLE_HEADER, fill: TABLE_HEADER },
...
function infoBox(text: string): Table {
  return new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    rows: [new TableRow({
      children: [new TableCell({
        ...
        borders: {
          top:    { color: '3B82F6', size: 8, style: BorderStyle.SINGLE },
          bottom: { color: '3B82F6', size: 8, style: BorderStyle.SINGLE },
          left:   { color: '3B82F6', size: 20, style: BorderStyle.SINGLE },
          right:  { color: '3B82F6', size: 8, style: BorderStyle.SINGLE },
        },
...
function buildCoverPage(analysis: RepoAnalysis, targetStack: string): Paragraph[] {
  ...
  return [
    ...
    new Paragraph({
      children: [new TextRun({ text: 'CODE MIGRATION REPORT', bold: true, size: 48, color: BRAND_BLUE, characterSpacing: 100 })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
    }),
    new Paragraph({
      children: [new TextRun({ text: `${repoInfo.owner} / ${repoInfo.repo}`, size: 36, color: ACCENT_TEAL, bold: true })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 120 },
    }),
    new Paragraph({
      children: [new TextRun({ text: `${detectedStack.framework || detectedStack.primaryLanguage}  →  ${targetStack}`, size: 28, color: TEXT_MUTED, italics: true })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 600 },
    }),
    new Paragraph({
      border: { top: { color: 'E2E8F0', size: 6, style: BorderStyle.SINGLE }, bottom: { color: 'E2E8F0', size: 6, style: BorderStyle.SINGLE } },
      children: [],
    ...
    ...[
      ...
      new Paragraph({
        children: [
          new TextRun({ text: `${label}:  `, bold: true, color: TEXT_DARK, size: 20 }),
          new TextRun({ text: value, color: TEXT_MUTED, size: 20 }),
        ],
        alignment: AlignmentType.CENTER,
...
function tokensToElements(tokens: Token[]): (Paragraph | Table)[] {
    ...
    switch (token.kind) {
      ...
      case 'table':
        if (token.headers.length > 0) {
          ...
          elements.push(new Paragraph({ children: [], spacing: { before: 100, after: 100 } }));
        }
        break;
      case 'bullet':
    ...
    sections: [{
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            children: [
              new TextRun({ text: `${analysis.repoInfo.owner}/${analysis.repoInfo.repo}  ·  Migration Report`, color: TEXT_MUTED, size: 16 }),
            ],
      ...
      footers: {
        default: new Footer({
          children: [new Paragraph({
            children: [
              new TextRun({ text: 'Generated by Code Migration Assistant  ·  Page ', size: 16, color: TEXT_MUTED }),
...
function buildToc(markdown: string): string {
  ...
  for (const e of entries) {
    if (e.level === 1) {
      if (inL3) { toc += '</ol>'; inL3 = false; }
      if (inL2) { toc += '</ol></li>'; inL2 = false; }
      toc += `<li><a href="#${e.id}">${escapeHtml(e.text)}</a>`;
      inL2 = false;
    } else if (e.level === 2) {
      if (inL3) { toc += '</ol>'; inL3 = false; }
      if (!inL2) { toc += '<ol class="toc-l2">'; inL2 = true; }
      toc += `<li><a href="#${e.id}">${escapeHtml(e.text)}</a>`;
      inL3 = false;
    } else {
      if (!inL3) { toc += '<ol class="toc-l3">'; inL3 = true; }
...
function buildSidebarNav(markdown: string): string {
  const lines = markdown.split('\n');
  const links: string[] = [];
